...
def route_prompt(skill_root: Path, scenario: dict[str, Any]) -> str:
    ...
    fixture_note = f" A read-only input fixture is available at {skill_root / fixture}." if fixture else ""
    return (
        ...
        "Do not solve the request. Return only the routing evaluation object required by the output schema.\n\n"
        f"User request: {scenario['request']}.{fixture_note}"
    )
